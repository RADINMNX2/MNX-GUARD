...
هر push روی شاخهٔ `main` گردش‌کار [`build.yml`](../.github/workflows/build.yml) را راه می‌اندازد. این گردش‌کار هستهٔ Rust را با `cargo-ndk` برای هر دو ABI کامپایل می‌کند، APK دیباگ را می‌سازد و آن را به‌عنوان artifact با نام `MNX GUARD` آپلود می‌کند. ساده‌ترین راه گرفتن یک بیلد تازه همین است، بدون اینکه لازم باشد کل زنجیرهٔ ابزار را روی سیستم خودتان نصب کنید.
