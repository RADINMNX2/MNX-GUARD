...
برنامه MNX GUARD یک کلاینت VPN بومی اندروید است که کل ترافیک گوشی را از پنج مسیر مستقل عبور می‌دهد. فرقش با اپ‌های پروکسی این است که آن‌ها معمولاً فقط مرورگر را پوشش می‌دهند؛ اینجا کار در سطح `VpnService` خود اندروید انجام می‌شود. یعنی یک اینترفیس TUN ساخته می‌شود و هر بستهٔ TCP و UDP و QUIC — از هر اپی که روی گوشی نصب است — از داخل تونل رد می‌شود.
...
| سرویس `MnxGuardVpnService` | چرخهٔ حیات VPN، ساخت TUN، سرپرستی ترابری |
| سرویس `MnxGuardTileService` | کاشی تنظیمات سریع |
...
آخرین APK را می‌توانید از [بخش Releases](https://github.com/RADINMNX2/MNX-GUARD/releases) یا از artifact های [Actions](https://github.com/RADINMNX2/MNX-GUARD/actions) بگیرید. نسخهٔ فعلی `1.7.18` است و خود برنامه هم آپدیت را از همان صفحهٔ Releases چک می‌کند.

| معماری دستگاه | فایل |
|---|---|
| معماری ARM ۶۴ بیتی، یعنی اکثر گوشی‌های امروزی | `MNX-GUARD-v1.7.18-arm64-v8a.apk` |
| معماری ARM ۳۲ بیتی، دستگاه‌های قدیمی‌تر | `MNX-GUARD-v1.7.18-armeabi-v7a.apk` |
...
هر push روی شاخهٔ `main` گردش‌کار [`build.yml`](.github/workflows/build.yml) را اجرا می‌کند و APK هر دو معماری را به‌عنوان artifact آپلود می‌کند.
