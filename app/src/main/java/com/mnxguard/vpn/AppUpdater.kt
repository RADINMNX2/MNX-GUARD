...
class AppUpdater(private val activity: Activity) {
                    ...
                    .onSuccess { release ->
                        when {
                            ...
                            !isNewer(release.version, appVersion()) ->
                                showMessage(Strings.t("You're up to date"), Strings.tf("MNX GUARD %s is installed", appVersion()))
    ...
    private fun announceUpdate(release: Release) {
        dialogBuilder()
            ...
            .setMessage(
                Strings.tf("MNX GUARD %s has been released. You are on %s.", release.version, appVersion()) + "\n\n" +
    ...
    private fun latestRelease(): Release? {
        val connection = (URL(RELEASE_URL).openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("User-Agent", "MNX-GUARD-Android")
        }
        try {
    ...
    private fun dialogBuilder() = MaterialAlertDialogBuilder(
        activity,
        if (AppAppearance.isNight(activity)) R.style.ThemeOverlay_MnxGuard_AlertDialog
        else R.style.ThemeOverlay_MnxGuard_Light_AlertDialog,
    )
    ...
    private companion object {
        ...
        const val RELEASE_URL = "https://$RELEASE_HOST/repos/RADINMNX2/MNX-GUARD/releases/latest"
        const val RELEASES_PAGE_URL = "https://github.com/RADINMNX2/MNX-GUARD/releases/latest"
