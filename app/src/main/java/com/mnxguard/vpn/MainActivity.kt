...
class MainActivity : Activity() {
    ...
    private val statusReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            intent.getStringExtra(MnxGuardVpnService.EXTRA_EXIT_IP)?.let { ip ->
                if (ip.isNotBlank()) {
                    coreExitIp = ip
                    coreExitCountry = ""
            ...
            if (intent.hasExtra(MnxGuardVpnService.EXTRA_TRAFFIC_TX)) {
                trafficTx = intent.getLongExtra(MnxGuardVpnService.EXTRA_TRAFFIC_TX, 0)
                trafficRx = intent.getLongExtra(MnxGuardVpnService.EXTRA_TRAFFIC_RX, 0)
                trafficSpeedTx = intent.getLongExtra(MnxGuardVpnService.EXTRA_TRAFFIC_SPEED_TX, 0)
            ...
            when (intent.getStringExtra(MnxGuardVpnService.EXTRA_STATUS)) {
                MnxGuardVpnService.STATUS_CONNECTING -> showConnecting(
                    intent.getStringExtra(MnxGuardVpnService.EXTRA_DETAIL),
                    intent.getIntExtra(MnxGuardVpnService.EXTRA_PROGRESS, -1),
                )
                MnxGuardVpnService.STATUS_STARTING -> showStarting()
                MnxGuardVpnService.STATUS_SCANNING -> showScanning()
                ...
                MnxGuardVpnService.STATUS_CONNECTED -> beginVerification()
                MnxGuardVpnService.STATUS_FAILED -> {
                    ...
                    // is shown normally.
                    if (!advanceAutoScan()) {
                        showFailure(intent.getStringExtra(MnxGuardVpnService.EXTRA_DETAIL))
                    }
                }
                MnxGuardVpnService.STATUS_DISCONNECTED -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!AppAppearance.isNight(this)) setTheme(R.style.Theme_MnxGuard_Light)
        ...
        footerWave = OrbitFooterWave(this, palette, Strings.t("SECURED BY MNX GUARD"))
    ...
    override fun onStart() {
        super.onStart()
        val filter = IntentFilter(MnxGuardVpnService.ACTION_STATUS)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
    ...
    private fun showOpeningOverlay() {
        ...
        val logo = ImageView(this).apply {
            setImageResource(R.drawable.mnxguard_splash_logo)
    ...
    private fun failFakeConnection() {
        ...
        startService(Intent(this, MnxGuardVpnService::class.java)
            .setAction(MnxGuardVpnService.ACTION_DISCONNECT))
    ...
    private fun createHeader(): LinearLayout = LinearLayout(this).apply {
        ...
        addView(label(Strings.t("MNX-GUARD"), 13f, MUTED, TypefaceStyle.MEDIUM).apply {
    ...
    private fun refreshPublicIp(resetRetry: Boolean = true) {
        ...
        if (coreExitIp.isBlank() && isTunnelActive()) {
            MnxGuardVpnService.lastMeasuredExitIp()
    ...
    private fun updateNotificationHealth(ip: String? = null, ping: String? = null, country: String? = null) {
        ...
        startService(Intent(this, MnxGuardVpnService::class.java)
            .setAction(MnxGuardVpnService.ACTION_NOTIFICATION_HEALTH)
            .apply {
                ip?.let { putExtra(MnxGuardVpnService.EXTRA_NOTIFICATION_IP, it) }
                ping?.let { putExtra(MnxGuardVpnService.EXTRA_NOTIFICATION_PING, it) }
                country?.let { putExtra(MnxGuardVpnService.EXTRA_NOTIFICATION_COUNTRY, it) }
            })
    }
    ...
    private fun copyFullLog() {
        Thread({
            ...
            runOnUiThread {
                ...
                clipboard?.setPrimaryClip(ClipData.newPlainText(Strings.t("MNX GUARD log"), text))
                    ...
                    startActivity(
                        Intent.createChooser(
                            Intent(Intent.ACTION_SEND)
                                .setType("text/plain")
                                .putExtra(Intent.EXTRA_SUBJECT, "MNX GUARD log")
    ...
    private fun openModeScreen() {
        ...
        content.addView(label(Strings.t("Choose how MNX GUARD connects"), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })
    ...
    private fun openSettingsScreen(animate: Boolean = true) {
        ...
        val autoReconnectRow = createToggleRow(
            ...
            preferences().edit().putBoolean(MnxGuardVpnService.AUTO_RECONNECT_PREF, it).apply()
        }
        ...
        val lanBypassRow = createToggleRow(
            ...
            preferences().edit().putBoolean(MnxGuardVpnService.LAN_BYPASS_PREF, it).apply()
        }
        ...
        content.addView(navRow(Strings.t("Source on GitHub"), iconRes = R.drawable.ic_github) {
            openLink("https://github.com/RADINMNX2/MNX-GUARD")
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8) })
        ...
        content.addView(label("MNX GUARD ${appVersion()}", 11.5f, Sculpt.withAlpha(MUTED, 0.7f)).apply {
            gravity = Gravity.CENTER_HORIZONTAL
    ...
    private fun chooseTunnelType() {
        ...
        showChoiceSheet(
            ...
            subtitle = Strings.t("How MNX GUARD carries your traffic"),
    ...
    private fun openTrafficMonitorScreen() {
        ...
        content.addView(label(Strings.t("Traffic carried by MNX GUARD. Per-app attribution is not available from encrypted tunnel counters."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })
    ...
    private fun openSplitTunnelScreen() {
        ...
        content.addView(header)
        content.addView(label(Strings.t("Choose which apps use MNX GUARD. Changes apply next connection."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private fun toggleTunnel() {
        ...
        if (TunnelStatus.isActive() || visualState == OrbitDialView.State.CONNECTING) {
            ...
            startService(Intent(this, MnxGuardVpnService::class.java).setAction(MnxGuardVpnService.ACTION_DISCONNECT))
    ...
    private fun connect(config: String) {
        ...
        // scan is running.
        armAutoScanWatchdog()
        startForegroundService(Intent(this, MnxGuardVpnService::class.java)
            .setAction(MnxGuardVpnService.ACTION_CONNECT)
            .putExtra(MnxGuardVpnService.EXTRA_CONFIG, config))
    }
    ...
    private fun configJson(): String {
        ...
        return if (chained) {
            CoreConfig.json(this, MnxGuardVpnService.CHAIN_PROTOCOL_MARKER.lowercase())
        } else {
    ...
    private fun startSessionTimer(restored: Boolean) {
        val serviceStart = MnxGuardVpnService.connectedSinceElapsed()
    ...
    private fun autoReconnectEnabled(): Boolean = preferences().getBoolean(
        MnxGuardVpnService.AUTO_RECONNECT_PREF,
        MnxGuardVpnService.AUTO_RECONNECT_DEFAULT,
    )
     ...
     * tunnel is up. [MnxGuardVpnService.lanBypassEnabled] is still the only
    ...
    private fun lanBypassEnabled(): Boolean = preferences().getBoolean(
        MnxGuardVpnService.LAN_BYPASS_PREF,
    ...
    private fun splitTunnelSummary(): String {
        ...
        return when (settings.mode()) {
            SplitTunnelSettings.Mode.ALL -> Strings.t("All apps use MNX GUARD")
