...
object CoreConfig {
     ...
     * absent. See [MnxGuardVpnService.raiseOuterLeg].
     *
     * Written only after the byte threshold in
     * [MnxGuardVpnService.recordWorkingPlainTransport] — a handshake is not
     ...
     * [MnxGuardVpnService.armRegionPhase]. A plain Psiphon connect always takes
     ...
     * the dead `if (!psiphonVpnMode)` arm in `MnxGuardVpnService.onConnected` is what
    ...
    fun portRejection(port: Int): String? = when {
        ...
        port in RESERVED_PORTS -> "$port is used internally by MNX GUARD"
        else -> null
    }
